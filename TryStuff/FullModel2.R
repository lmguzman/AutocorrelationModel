...
#Equations:
#  Nt+1'= Nt*R*exp(-Nt/K)*exp(-sum(a*Pt))
...
predpreymanypred<-function(prednum=2,patch=1000,N0=10, P0=5,con=c(0.4,0.4), a=c(0.3,0.3), m1=0.1,m2=c(0.1,0.1),w=c(1,2), K=10^10, numruns=5000, gens=10, mn=-0.1,t=10^-20,rho=0.5,std=0.7)
{
  library(abind)
...
predpreymanypred2<-function(prednum=2,patch=1000,N0=10, P0=5,con=c(0.4,0.4), a=c(0.3,0.3), m1=0.1,m2=c(0.1,0.1),w=c(1,2), K=10^10, numruns=5000, gens=10, mn=-0.1,t=10^-20,rho=0.5,std=0.7)
{
  library(abind)
  #creates an initial matrix and array to keep the prey and predator densities 
  allrand<-array(rnorm(patch*numruns*(gens+1),0,1), dim=c(patch,numruns,(gens+1)))
  ...
  for(i.gens in 1:gens)
  {
    ...
    #Calculate the predator burden for the prey (that is *exp(-a*ppops)* for every predator)
    prev.pred<-sweep(ippops,MARGIN=3,a,FUN='*')
    pred.burden<-exp(-1*colSums(aperm(prev.pred, c(3,1,2))))
    ...
    prev.pred2<-sweep(prev.pred1,MARGIN=3,con,FUN='*')
    #Nt*ci*(1-exp(-ai*Pit))
    prev.pred4<-array(apply(prev.pred2,MARGIN=3,FUN=function(x){x*ipops}),dim=c(patch,numruns,prednum))
    ...
    ippops.new[ippops.new<t]<-0
    #migration
    cosum<-matrix(colSums(ipops.new),nrow=patch,ncol=numruns,byrow=T)
    ipops<-(1-m1)*ipops.new+(m1/(patch-1))*(cosum-ipops.new)
